...
void LaserPointer::begin(const paintcore::Point &point, bool right, float zoom)
{
	...
	msgs << protocol::MessagePtr(new protocol::LaserTrail(owner.myId(),
		owner.activeBrush().color().rgb(),
		m_persistence
	));
	msgs << protocol::MessagePtr(new protocol::MovePointer(owner.myId(), point.x() * 4, point.y() * 4));
...
void LaserPointer::motion(const paintcore::Point &point, bool constrain, bool center)
{
	Q_UNUSED(constrain);
	Q_UNUSED(center);
	owner.sendMessage(protocol::MessagePtr(new protocol::MovePointer(owner.myId(), point.x() * 4, point.y() * 4)));
}

void LaserPointer::end()
{
	owner.sendMessage(protocol::MessagePtr(new protocol::LaserTrail(owner.myId(), 0, 0)));
}
