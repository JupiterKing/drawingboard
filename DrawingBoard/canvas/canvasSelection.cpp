...
canvasSelection::canvasSelection(QObject *parent)
	: QObject(parent), m_closedPolygon(false)
{

}

void canvasSelection::saveShape()
{
...
bool canvasSelection::isTransformed() const
{
	return m_shape != m_originalShape;
}

void canvasSelection::reset()
{
	m_shape = m_originalShape;
	emit shapeChanged(m_shape);
}

void canvasSelection::resetShape()
{
...
void canvasSelection::setShape(const QPolygonF &shape)
{
	m_shape = shape;
...
void canvasSelection::setShapeRect(const QRect &rect)
{
	setShape(QPolygonF({
		rect.topLeft(),
...
void canvasSelection::translate(const QPoint &offset)
{
...
void canvasSelection::scale(qreal x, qreal y)
{
	if(x==1.0 && y==1.0)
		return;
...
canvasSelection::Handle canvasSelection::handleAt(const QPointF &point, float zoom) const
{
...
void canvasSelection::beginAdjustment(Handle handle)
{
	m_adjustmentHandle = handle;
	m_preAdjustmentShape = m_shape;
}

void canvasSelection::adjustGeometry(const QPoint &delta, bool keepAspect)
{
	if(keepAspect) {
...
void canvasSelection::adjustScale(int dx1, int dy1, int dx2, int dy2)
{
...
void canvasSelection::adjustRotation(float angle)
{
	Q_ASSERT(m_preAdjustmentShape.size() == m_shape.size());
...
void canvasSelection::adjustShear(float sh, float sv)
{
	Q_ASSERT(m_preAdjustmentShape.size() == m_shape.size());
...
void canvasSelection::addPointToShape(const QPointF &point)
{
	if(m_closedPolygon) {
...
void canvasSelection::closeShape()
{
	if(!m_closedPolygon) {
		m_closedPolygon = true;
		saveShape();
...
bool canvasSelection::isAxisAlignedRectangle() const
{
	if(m_shape.size() != 4)
		return false;
...
QImage canvasSelection::shapeMask(const QColor &color, QRect *maskBounds) const
{
	return tools::SelectionTool::shapeMask(color, m_shape, maskBounds);
}

void canvasSelection::setPasteImage(const QImage &image)
{
	m_moveRegion = QPolygonF();
	setPasteOrMoveImage(image);
}

void canvasSelection::setMoveImage(const QImage &image, const QSize &canvasSize)
{
...
void canvasSelection::setPasteOrMoveImage(const QImage &image)
{
...
QList<protocol::MessagePtr> canvasSelection::pasteOrMoveToCanvas(uint8_t contextId, int layer) const
{
...
QList<protocol::MessagePtr> canvasSelection::fillCanvas(uint8_t contextId, const QColor &color, paintcore::BlendMode::Mode mode, int layer) const
{
	QRect area;
	QImage mask;
	QRect maskBounds;
